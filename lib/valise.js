/*
 * valise
 * https://github.com/jrussell-ivantage/valise
 *
...
module.exports = function( confPath ) {
	...
	if( 2 !== parts.length ) {
		if( 1 === parts.length ) {
			...
			throw new Error( "Valise got a bad resource path... what am I supposed to do with with: " + confPath + "?" );
		}
	} else {
		dir = parts[0];
		resource = parts[1];
	...
	if( !confRoots.hasOwnProperty( dir ) ) {
		throw new Error( "Valise doesn't have a resource path registered for: " + dir );
	}
